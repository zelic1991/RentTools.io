...
const ZELIC_CALENDAR_THEME = {
  ...
  "--m-accent": "#7B2E62",
  "--m-accent-2": "#9A3B72",
  "--cleaning-cell-bg": "#F8EFF4",
  "--cleaning-fg": "#7B2E62",
  "--cleaning-bg": "#F8EFF4",
  "--cleaning-border": "#E2CDDB",
} as CSSProperties;
...
export function MobileCalendar({ data }: { data: MobileOperationsData }) {
  ...
  return (
    <section
      ...
      className="space-y-4 text-[#2B241D]"
      style={ZELIC_CALENDAR_THEME}
    >
      <div className="flex items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#7B2E62]">Kalender</p>
        ...
        <button
          ...
          className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#8F7E65] bg-[#F8EFF4] px-3 text-sm font-medium text-[#3F1735] shadow-sm outline-none hover:bg-[#E8C7DC] focus-visible:ring-2 focus-visible:ring-[#7B2E62]"
        >
          <RotateCcw aria-hidden className="h-4 w-4" /> Heute
        </button>
      </div>

      <div className="overflow-hidden rounded-2xl border border-[#D6C8AE] bg-[#F6F1E6] shadow-sm">
        <div className="flex min-h-14 items-center justify-between border-b border-[#D6C8AE] bg-[#ECE1CC] px-2 sm:px-4">
          <button
            type="button"
            aria-label="Vorheriger Monat"
            onClick={() => moveMonth(-1)}
            disabled={activeKey <= minMonth}
            className="inline-flex h-11 w-11 items-center justify-center rounded-xl text-[#3F1735] outline-none hover:bg-[#F8EFF4] focus-visible:ring-2 focus-visible:ring-[#7B2E62] disabled:opacity-30"
          >
            <ChevronLeft aria-hidden className="h-5 w-5" />
          </button>
          <div className="flex items-center gap-2 text-base font-semibold capitalize">
            <CalendarDays aria-hidden className="h-4 w-4 text-[#7B2E62]" />
        ...
        <div className="grid grid-cols-7 border-b border-[#D6C8AE] bg-[#F8EFF4] px-1 py-2 text-center text-[11px] font-semibold uppercase tracking-wide text-[#685C4B] sm:px-2">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs text-[#685C4B] sm:grid-cols-4">
        <div className="flex min-h-11 items-center gap-2 rounded-xl border border-[#D6C8AE] bg-[#F6F1E6] px-3"><span className="h-3 w-3 rounded-full border border-[#8F7E65] bg-[#F6F1E6]" /> Leer = frei</div>
        <div className="flex min-h-11 items-center gap-2 rounded-xl border border-[#D6C8AE] bg-[#F6F1E6] px-3"><span className="h-3 w-3 rounded-full bg-[#FF385C]" /> Airbnb</div>
        <div className="flex min-h-11 items-center gap-2 rounded-xl border border-[#D6C8AE] bg-[#F6F1E6] px-3"><span className="h-3 w-3 rounded-full bg-[#003580]" /> Booking</div>
        <div className="flex min-h-11 items-center gap-2 rounded-xl border border-[#D6C8AE] bg-[#F6F1E6] px-3"><span className="h-3 w-3 rounded-full bg-[#685C4B]" /> Sonstige</div>
      </div>
      <p className="text-xs text-[#685C4B]">Ein Checkout-Tag bleibt für eine neue Anreise am selben Tag verfügbar. Nur Reservierungen, manuelle Sperren oder bewusst gesetzte Puffertage blockieren.</p>
      ...
      <section aria-labelledby="month-reservations-heading" className="space-y-2">
        ...
        {monthReservations.length === 0 ? (
          <p className="rounded-xl border border-dashed border-[#8F7E65] bg-[#F6F1E6]/70 px-4 py-3 text-sm text-[#685C4B]">Keine Reservierung in diesem Monat.</p>
        ) : (
            ...
            const content = (
              <>
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-[#685C4B]">{reservation.checkIn.slice(0, 10)}–{reservation.checkOut.slice(0, 10)}</span>
                </span>
                <span className="text-xs font-medium text-[#685C4B]">{reservation.platform}</span>
              </>
            );
            const className = "flex min-h-12 items-center gap-3 rounded-xl border border-[#D6C8AE] bg-[#F6F1E6] px-4 text-sm shadow-sm";
            return data.canWrite ? (
              <Link key={reservation.id} href={`/dashboard?property=${data.selectedProperty.id}&reservation=${reservation.id}&view=guests`} className={`${className} outline-none hover:border-[#9A3B72] focus-visible:ring-2 focus-visible:ring-[#7B2E62]`}>
                {content}
              </Link>
            ) : (
